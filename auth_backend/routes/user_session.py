...
from auth_backend.routes.models.models import UserGroups, UserIndirectGroups, UserInfo, UserGet, UserScopes, SessionScopes
...
async def me(
    session: UserSession = Depends(UnionAuth(scopes=[], allow_none=False, auto_error=True)),
    info: list[Literal["groups", "indirect_groups", "scopes", "user_scopes", ""]] = Query(default=[]),
) -> dict[str, str | int]:
    if session.expired:
    ...
    if "scopes" in info:
        result = (
            result
            | SessionScopes(
                session_scopes=list(session.scopes),
            ).dict()
        )

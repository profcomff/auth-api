...
async def already_exists_handler(req: starlette.requests.Request, exc: AlreadyExists):
    return JSONResponse(content=ResponseModel(status="Error", message=f"{exc}").json(), status_code=409)
...
async def http_error_handler(req: starlette.requests.Request, exc: Exception):
    return JSONResponse(content=ResponseModel(status="Error", message="Internal server error").json(), status_code=500)

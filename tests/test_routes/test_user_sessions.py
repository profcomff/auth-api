...
def test_create_session(client_auth: TestClient, dbsession: Session, user_scopes):
    ...
    current_session_scopes_get = client_auth.get("/me?info=session_scopes", headers=header)
    ...
    logger.error(current_session_scopes_get.json())
    ...
    dbsession.query(UserSessionScope).filter(UserSessionScope.scope_id == scope1.id,
                                             UserSessionScope.scope_id == scope2.id).delete()

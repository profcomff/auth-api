...
def test_unprocessable_jsons_with_token(client: TestClient, dbsession: Session, user):
    ...
    response = client.post(
        f"{url}{user_id}/request", headers={"token": auth_token}, json={"password": "", "new_password": "changed"}
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    response = client.post(
        f"{url}{user_id}/request", headers={"token": ""}, json={"password": "", "new_password": "changed"}
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    response = client.post(
        f"{url}{user_id}/request", headers={"token": ""}, json={"password": body["password"], "new_password": "changed"}
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    response = client.post(
        f"{url}{user_id}/request", headers={"token": ""}, json={"password": body["password"], "new_password": ""}
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    response = client.post(
        f"{url}{user_id}/request",
...
def test_no_token(client: TestClient, dbsession: Session, user_id: str):
    ...
    response = client.post(
        f"{url}{user_id}", headers={"reset-token": reset_token.value + "x"}, json={"new_password": "changedstring2"}
    )
...
def test_with_token(client: TestClient, dbsession: Session, user):
    ...
    response = client.post(
        f"{url}{user_id}/request", headers={"token": auth_token}, json={"password": "wrong", "new_password": "changed"}
    )
    assert response.status_code == status.HTTP_401_UNAUTHORIZED

    response = client.post(
        f"{url}{user_id}/request",
        headers={"token": auth_token + "wrong"},
        json={"password": body["password"], "new_password": "changed"},
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN

...
1. Дернуть ручку `POST /email/reset/password/restore`. Вы передаете `{email: ""}`в нагрузке
2. Вам придет письмо, где будет ссылка НА ФРОНТ(надо сделать это), в ссылке будет reset_token
3. Токен надо передать в ручку `POST /email/reset/password` в заголовках, вместе с `{new_password: ""}` и пароль будет изменен

### Email: Изменение пароля
1. Если пароль не забыт, а просто надо его поменять. Тогда в `POST /email/reset/password/request` передается токен авторизации, в теле вы передаете `{password: "", new_password: ""}`
...
### Созданиие группы 
1. Дернуть ручку `POST /group`, передать `{"name" - имя группы, "parent_id" - айди предка группы(из него подтянутся скоупы), "scopes" - скуопы группы}`.
...
1. Дернуть `PATCH /group/{id}`, передать `{scopes: [<new_scopes>]}`, где <new_scopes> - список, содержащий старые скоупы + новые. Ручка полностью заменит скоупы на переданные

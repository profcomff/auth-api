...
- `TELEGRAM_BOT_ID` - ID бота приложения; можно получить через aiogram или телеграм бота @getmyid_bot
...
1. Необходимо создать виджет на странице приложения по правилам телеграма. Пример:
```html
<script async 
src="https://telegram.org/js/telegram-widget.js?21" 
data-telegram-login="fizfak_bot" 
data-size="large" 
data-auth-url="TELEGRAM_REDIRECT_URL"></script>
```
*Можно также посмотреть пример на официальном сайте телеграма https://core.telegram.org/widgets/login*
2. Пользователь нажимает на виджет и переходит на страницу телеграма, где принимает соглашение, затем его возвращает на страницу, которую можно узнать запросом `GET /telegram/redirect_url`
3. Если телеграм успешно передал GET параметры пользователя, то проверяем их на достоверность методом `/telegram/check`.
4. Если данные пользователя оказались достоверными, то записываем данные пользователя в базу данных. В случае недостоверности, показываем экран ошибки авторизации.


### Создание скоупсов 
0. У вас должен быть аккаунт с правом на создание скоупсов :)
1. Получаете токен со скоупом auth.scope.create 
2. Идете с токеном на ручку ` POST /scope`, отправляете его туда в заголовке и в теле отправляете `{name: str - имя скоупа(там должно быть две точки и три слова без пробелов), comment - комментарий}
3. Профит
